...
(() => {
  ...
  // ---------- Shopping mode + listener manager ----------
  ...
  function wireAll(){
    ...
    for (const t of ["click","submit","pointerup","touchend","keydown"]) {
      on(document, t, handleAfter, true);
    }
  ...
  function findProductNode(node) {
    try {
      ...
      const t = node["@type"];
      const isProduct = Array.isArray(t) ? t.map(lower).includes("product") : lower(t) === "product";
  ...
  // Initial attempt to wire (guarded). Storage watcher will flip if needed.
